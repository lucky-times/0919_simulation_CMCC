           ...
           for rr = 1:N_RB
                   ...
                   for u_ = 1:N_UE
                       if ~strcmp(attached_UEs(user_ind(u_)).traffic_model.type,'fullbuffer') 
                            ...
                            if  bitnr <= 0
                                ...
                                metric(res(r_),u_) = min(c(res(r_),u_)*12*7,bitnr)*attached_UEs(user_ind(u_)).lambda;
                            end
                       else   
                            metric(res(r_),u_) = c(res(r_),u_)*12*7*(max(obj.av_throughput(user_ind(u_)),eps)^alpha_tmp+attached_UEs(user_ind(u_)).lambda);  
                       end
                   end
               end
               maxi = max(metric(:));
               ...
               if ~strcmp(attached_UEs(user_ind(temp_ue)).traffic_model.type,'fullbuffer')
                   if isinf(attached_UEs(user_ind(temp_ue)).traffic_model.bit_count)
                       attached_UEs(user_ind(temp_ue)).traffic_model.coarse_decrease(c(temp_res,temp_ue)*(12*7-overhead),1);
                   else
                       attached_UEs(user_ind(temp_ue)).traffic_model.coarse_decrease(c(temp_res,temp_ue)*(12*7-overhead));
                   end
               end
           end
           RB_UEs = RB_UEs';
