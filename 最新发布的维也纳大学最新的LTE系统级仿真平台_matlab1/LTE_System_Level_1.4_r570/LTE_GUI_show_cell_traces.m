...
% hold on
% 
% for i = 1:size(simulation_traces.UE_traces,2)
%     SINR(i) = 10*log10(mean(mean(squeeze(10.^(simulation_traces.UE_traces(i).SINR(1,:,:)/10)))));
%     SNR(i) = simulation_traces.UE_traces(i).SNR(1);
%     sector(i) = simulation_traces.UE_traces(i).attached_sector(1);
% end
% sum_TP = zeros(max(sector),1);
% [SNRs indices] = sort(SNR(1:20));
% % indices
% % Plot eNodeB UEs (at the last TTI)
% for u_=1:length(UEs)
%     ind = find(indices == u_);
%     if (UEs(u_).attached_eNodeB.id==eNodeB_idx) && (UEs(u_).attached_sector==sector_idx)
%         scatter(UEs(u_).pos(1),UEs(u_).pos(2),'Marker','.','MarkerFaceColor','black','MarkerEdgeColor','black','SizeData',500);
%         switch u_
%             case 11
%                 text(UEs(u_).pos(1)-20,UEs(u_).pos(2)-25,num2str(ind),'Fontsize',20);%,'Color',[1,1,1]);
%             case 14
%                 text(UEs(u_).pos(1)-10,UEs(u_).pos(2)+25,num2str(ind),'Fontsize',20);%,'Color',[1,1,1]);
%             case 18
%                 text(UEs(u_).pos(1)-25,UEs(u_).pos(2)+5,num2str(ind),'Fontsize',20);%,'Color',[1,1,1]);
%             case 20
%                 text(UEs(u_).pos(1)-25,UEs(u_).pos(2)+5,num2str(ind),'Fontsize',20);%,'Color',[1,1,1]);
%             otherwise    
%                 text(UEs(u_).pos(1)+5,UEs(u_).pos(2)+5,num2str(ind),'Fontsize',20);%,'Color',[1,1,1]);
%         end
%     else
%         scatter(UEs(u_).pos(1),UEs(u_).pos(2),'Marker','.','MarkerFaceColor',[0.5,0.5,0.5],'MarkerEdgeColor',[0.5,0.5,0.5],'SizeData',500);
%     end
% end
% 
% % Plot eNodeBs
...
%         destiny = vector + origin;
% 
%         plot([origin(1) destiny(1)],[origin(2) destiny(2)],'Linewidth',2.5);
%     end
%     % Plot the eNodeBs
%     scatter(eNodeBs(b_).pos(1),eNodeBs(b_).pos(2),'Marker','o','MarkerFaceColor','red','MarkerEdgeColor','black','SizeData',100);
%     text(eNodeBs(b_).pos(1)+15*text_shifting,eNodeBs(b_).pos(2)+15*text_interline,[num2str(eNodeBs(b_).id)],'Fontsize',20);
% end
% set(gca,'Fontsize',24);
% axis square
% grid on
