...
LTE_config.always_on     = false; % Controls whether the eNodeB is always radiating power (dafault and worse-case scenario) or no power is used when no UEs are attached
...
switch LTE_config.network_source
    ...
    case 'capesso'
        ...
        LTE_config.macroscopic_pathloss_model_settings.environment = '';
        % eNodeB's transmit power, in Watts -> Read from file
        
        % Configure Capesso import params
        LTE_config.capesso_params.planning_tool              = 'capesso';
        LTE_config.capesso_params.pathloss_data_folder       = './non_public_data/LTE_trial/pathloss';
        
        LTE_config.capesso_params.enable_dtm                 = true;    % 'false' for debugging
        LTE_config.capesso_params.dtm_folder                 = './non_public_data/wien_dtm';
        LTE_config.capesso_params.dtm_file_name              = 'DTM_test_new_20m_wien.bil';
        LTE_config.capesso_params.dtm_hdr_file_name          = 'DTM_test_new_20m_wien.hdr'; 
        
        LTE_config.capesso_params.kathrein_antenna_folder    = './data_files/KATHREIN_antenna_files/txap';
        
        % Capesso Site 1
        % LTE_config.capesso_params.cell_atoll_filename        = 'cell_atoll.txt';
        % LTE_config.capesso_params.site_atoll_filename        = 'Site_atoll.txt';
        % LTE_config.capesso_params.transmitter_atoll_filename = 'transmitter_atoll.txt';
        
        % LTE Trial
        LTE_config.capesso_params.cell_atoll_filename        = 'LTE_cell_trial_cluster.txt';
        LTE_config.capesso_params.site_atoll_filename        = 'LTE_sites_trial_cluster.txt';
        LTE_config.capesso_params.transmitter_atoll_filename = 'LTE_BTS_trial_cluster_20100809.txt';
        ...
        LTE_config.capesso_params.eNodeB_ROI_increase_factor = 0.2; % Increase ROI relative to size
        ...
        % NOTE: are these parameters needed?
...
LTE_config.channel_model.trace_length = 30; % Length of the trace in seconds. Be wary of the size you choose, as it will be loaded in memory.
LTE_config.channel_model.correlated_fading = true;
% LTE_config.pregenerated_ff_file           = 'auto';
LTE_config.pregenerated_ff_file           = fullfile('./data_files','OLSM_2x2_1.4MHz_TU_capesso_tests.mat');
...
%% eNodeB antenna options (overridden by the loaded network data)
...
% This section refers to the generic use of Kathrein antennas without
% having additional information from network planning tool
if strcmp(LTE_config.antenna.antenna_gain_pattern, 'kathreinTSAntenna')
   ...
   LTE_config.antenna.kathrein_antenna_folder = './data_files/KATHREIN_antenna_files/txap';
   LTE_config.antenna.file_format = 'txap';
