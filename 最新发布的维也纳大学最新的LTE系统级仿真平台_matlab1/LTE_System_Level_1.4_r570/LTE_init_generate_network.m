...
% Mean antenna gain
max_antenna_gain    = LTE_config.antenna.max_antenna_gain;
...
for b_ = 1:length(eNodeBs)
    ...
    for s_ = 1:number_of_sectors
        ...
        switch antenna_gain_pattern
            case 'berger'
                eNodeBs(b_).sectors(s_).antenna = antennas.bergerAntenna(max_antenna_gain);
            case 'TS 36.942'
                eNodeBs(b_).sectors(s_).antenna = antennas.TS36942Antenna(max_antenna_gain);
            ...
            case 'kathreinTSAntenna'
                ...
                antenna_type            = LTE_config.antenna.antenna_type;
                frequency               = LTE_config.antenna.frequency;
                eNodeBs(b_).sectors(s_).antenna = antennas.kathreinTSAntenna(kathrein_antenna_folder, antenna_type, frequency,LTE_config.antenna.file_format);
                % Additional parameters
                eNodeBs(b_).altitude = LTE_config.site_altiude;
                eNodeBs(b_).sectors(s_).tx_height = LTE_config.site_height;
                eNodeBs(b_).sectors(s_).mechanical_downtilt = LTE_config.antenna.mechanical_downtilt;
                eNodeBs(b_).sectors(s_).electrical_downtilt = LTE_config.antenna.electrical_downtilt;
            otherwise
...
if (LTE_config.show_network>0) & ~strcmp(antenna_gain_pattern,'kathreinTSAntenna')
...
if  LTE_config.nr_eNodeB_rings ~= 0
...
if strcmp(antenna_gain_pattern,'kathreinTSAntenna')
    % 3D gain pattern used, also vertical angle grid needed
    for b_ = 1:length(eNodeBs)
        distances = sqrt((position_grid_meters(:,1)-eNodeBs(b_).pos(1)).^2 + (position_grid_meters(:,2)-eNodeBs(b_).pos(2)).^2);
        distance_matrix = reshape(distances,roi_height_pixels,roi_width_pixels);
        for s_ = 1:number_of_sectors
             % Horizontal angle grid
             angle_grid = rad2deg(atan2((position_grid_meters(:,2)-eNodeBs(b_).pos(2)),(position_grid_meters(:,1)-eNodeBs(b_).pos(1)))) - wrapTo360(-eNodeBs(b_).sectors(s_).azimuth+90); % Convert the azimuth (0°=North, 90°=East, 180^=South, 270°=West) degrees to cartesian
             horizontal_angle_grid   = reshape(angle_grid,roi_height_pixels,roi_width_pixels);
             horizontal_angle_grid_s = wrapTo360(horizontal_angle_grid);
             % Vertical angle grid
             rx_height = LTE_config.rx_height;
             tx_height = eNodeBs(b_).sectors(s_).tx_height;
             vertical_angle_grid_el = rad2deg(atan2((tx_height - rx_height), distance_matrix));
             % Calculate antenna gain
             sector_antenna_gain(:,:,s_,b_) = eNodeBs(b_).sectors(s_).antenna.gain(horizontal_angle_grid_s, vertical_angle_grid_el, eNodeBs(b_).sectors(s_).electrical_downtilt,  eNodeBs(b_).sectors(s_).mechanical_downtilt);
        end
    end
else
