classdef txapFileImporter
    % Reads out a given folder/s containing antenna .txap files and stores
    ...
    methods
        function obj=txapFileImporter(dirname,varargin)
            data_idx = 1;
            for folder_idx = 1:1+length(varargin)
                ...
                for i_=1:length(antenna_files)
                    ...
                    % STATIC - FURTHER DATA NEEDED !!!
                    % obj.antenna_data(data_idx).frequency = M3{2};
                    obj.antenna_data(data_idx).frequency = 2140; 
                    %
