function [ sector_capacity max_SINR_dB_all max_SNR_dB_all] = LTE_common_calculate_cell_capacity(networkPathlossMap,eNodeBs,CQI_mapper,varargin)
...
for b_=1:num_eNodeBs
    for s_=1:num_sectors
        ...
        SNR_linear_all(:,:,s_,b_)  = RX_powers_W(:,:,s_,b_) ./ (thermal_noise_W);
...
SNR_dB_all_sorted  = reshape(SNR_dB_all,size(SNR_dB_all,1),size(SNR_dB_all,2),[]);
SNR_dB_all_sorted  = sort(SNR_dB_all_sorted,3);
...
%     hold on;
    ...
    %% Plot CQIs
    subplot(2,2,2);
    mapped_CQIs = CQI_mapper.SINR_to_CQI(SINR_dB);
    % figure; hist(floor(mapped_CQIs(target_sector_area)),100);
    ...
    subplot(2,2,1);
    SINR_limits = [-5 20];
    imagesc(networkPathlossMap.roi_x,networkPathlossMap.roi_y,max_SINR_dB_all); % Easily modifiable to plot the SNR
    set(gca,'YDir','normal');
    if shadow_fading_used
    ...
    ylabel('y pos [m]');
    hold on;
%     if shadow_fading_used%         figure(6)%         imagesc(networkPathlossMap.roi_x,networkPathlossMap.roi_y,max_SINR_dB_all+10*log10(LTE_config.bandwidth)-10*log10(LTE_config.RB_bandwidth));%         colorbar;%         caxis([7 32]);% %         pause(0.05)%         set(gca,'YDir','normal');%         error('jetzt')%     end    
    % Plot target sector boundary
    if ~shadow_fading_used && ~real_data
