classdef winnerChannelFactory < channel_gain_wrappers.channelFactory
    ...
    methods (Static)
        ...
        function found = prepare_winner_channel_model_path(winner_model_path)
            ...
            if ~exist(fullfile(winner_model_path,'dipole.m'),'file')
                error('Please download the Winner+ II channel model from http://projects.celtic-initiative.org/winner+/Documents/Publications/WIM2_3D_ant_ver064_220908.zip and extract it into the "%s" folder',winner_model_path);
            else
    ...
    methods
        function obj = winnerChannelFactory(bandwidth,winner_params)
        ...
        function cache_filename = generate_cache_filename(obj)
            ...
            % params
            if winner_params.carrier_freq >= 1e9
                freq_string = sprintf('%3.1fGHz',winner_params.carrier_freq/1e9);
            else
                freq_string = sprintf('%3.0fMHz',winner_params.carrier_freq/1e6);
            end
            cache_filename = sprintf('winer_array_%dx%d_%s.mat',winner_params.BS_config.nTx,winner_params.UE_config.nRX,freq_string);
        end
        ...
        function Arrays = generate_winner_channel_model_antenna_array(obj)
            ...
            switch LTE_params.BS_config.nTx
                ...
                case 3
                    % Not defined
                    error('3 TX antenna configuration not defined.');
                    % pattern(1,:,1,:) = dipole(Az,0);
            ...
            switch LTE_params.UE_config.nRX
                ...
                case 2
                    ...
                    %Rotation = [0 0 90;0 0 90]; % Data card position??
                    pattern = zeros(2,2,1,length(Az));
                    %        pattern(1,:,1,:) = dipole(Az,0);
