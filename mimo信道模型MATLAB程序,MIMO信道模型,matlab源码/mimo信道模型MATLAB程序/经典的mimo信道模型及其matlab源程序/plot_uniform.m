...
for (k = 1:1:number_clusters)
  ...
  subplot(2,1,plot_number),plot(span, amplitude(k), 'b-');
  hold on;
  subplot(2,1,plot_number),line([cluster_min cluster_min],[0 amplitude(k)]);
  subplot(2,1,plot_number),line([AOA(k) AOA(k)],[0 amplitude(k)]);
  subplot(2,1,plot_number),line([cluster_MAX cluster_MAX],[0 amplitude(k)]);
end;
