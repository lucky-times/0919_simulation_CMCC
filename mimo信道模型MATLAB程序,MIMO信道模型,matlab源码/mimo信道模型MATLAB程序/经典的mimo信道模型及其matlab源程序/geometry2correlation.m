...
%
% (c) Laurent Schumacher, AAU-TKN/IES/KOM/CPK/CSys - September 2001
...
while ((Field ~= 0) & (Field ~=1))
    Field = input('Type of correlation coefficient (Field = 0, Envelope = 1)? ');
end;

% Configuration of Node B

string = sprintf('Node B');
  ...
  switch(PAS_type_Node_B)
   ...
   case 2
    plot_gaussian(cluster_number_Node_B, Q_Node_B, phi_deg_Node_B, ...
		  sigma_deg_Node_B, delta_phi_deg_Node_B, 1);
   case 3
...
if (display & (~(N==1)))
  ...
  switch(PAS_type_UE)
   case 1
    plot_uniform(cluster_number_UE, Q_UE, phi_deg_UE, AS_deg_UE, 2);
   case 2
