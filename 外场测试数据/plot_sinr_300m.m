...
close all;
%% 加载UAV信息
str = '7.4.2多层窄波束配置--蜂窝三扇区组网/7.4.2多层窄波束配置--蜂窝三扇区组网-1km站间距-飞行高度300米-加扰50%.csv';
data = readtable(str);
info = data(:, [3,4,5,10,8]);
info = table2array(info);
temp = info((info(:, 1)>=280 & info(:, 1)<=325), :);% 高度、经度、纬度、SINR值
...
if(labelMode == 'English')
    ...
    ylabel('SINR/dB')
    str = sprintf('SINR Calculation Based on Field Data, h = 300m, r = 100m');
elseif(labelMode == 'Chinese')
    xlabel('无人机飞行序列号')
    ylabel('SINR/(dB)');
    str = sprintf('实测数据SINR, h = 300m, r = 100m');
end  
title(str)
...
if(labelMode == 'English')
    xlabel('UAV flight sequence number')
    ylabel('SINR/dB')
    str = sprintf('SINR Calculation Based on Field Data, h = 300m, r = 300m');
elseif(labelMode == 'Chinese')
    xlabel('无人机飞行序列号')
    ylabel('SINR/(dB)');
    str = sprintf('实测数据SINR, h = 300m, r = 300m');
end  
title(str)
...
if(labelMode == 'English')
    xlabel('UAV flight sequence number')
    ylabel('SINR/dB')
    str = sprintf('SINR Calculation Based on Field Data, h = 300m, r = 500m');
elseif(labelMode == 'Chinese')
    xlabel('无人机飞行序列号')
    ylabel('SINR/(dB)');
    str = sprintf('实测数据SINR, h = 300m, r = 500m');
end  
title(str)
...
if(labelMode == 'English')
    xlabel('UAV flight sequence number')
    ylabel('SINR/dB')
    str = sprintf('SINR Calculation Based on Field Data, h = 300m, r = 800m');
elseif(labelMode == 'Chinese')
    xlabel('无人机飞行序列号')
    ylabel('SINR/(dB)');
    str = sprintf('实测数据SINR, h = 300m, r = 800m');
end  
title(str)
...
if(labelMode == 'English')
    xlabel('UAV flight sequence number')
    ylabel('SINR/dB')
    str = sprintf('SINR Calculation Based on Field Data, h = 300m, r = 1200m');
elseif(labelMode == 'Chinese')
    xlabel('无人机飞行序列号')
    ylabel('SINR/(dB)');
    str = sprintf('实测数据SINR, h = 300m, r = 1200m');
end  
title(str)
