...
plot(1:length(plot_Sinr), diff, 'k-o', LineWidth=linewidth2);
mean1 = mean(diff);
...
if(labelMode == 'English')
    ...
    ylabel('SINR/(dB)')
    str = sprintf('Comparison of SINR for UAV at an Altitude of 200m and a Radius of 100m');
    legend('Measured', 'Simulated');
elseif(labelMode == 'Chinese')
    xlabel('无人机飞行序列号')
    ylabel('SINR/(dB)');
    str = sprintf('UAV在200m高度，100m半径下的SINR比较');
    legend('实测', '仿真');
end
title(str)
grid on;
...
if(labelMode == 'English')
    xlabel('UAV flight sequence number')
    ylabel('SINR/(dB)')
    str = sprintf('Comparison of SINR for UAV at an Altitude of 200m and a Radius of 300m');
    legend('Measured', 'Simulated');
elseif(labelMode == 'Chinese')
    xlabel('无人机飞行序列号')
    ylabel('SINR/(dB)');
    str = sprintf('UAV在200m高度，300m半径下的SINR比较');
    legend('实测', '仿真');
end
title(str)
grid on;
...
if(labelMode == 'English')
    xlabel('UAV flight sequence number')
    ylabel('SINR/(dB)')
    str = sprintf('Comparison of SINR for UAV at an Altitude of 200m and a Radius of 500m');
    legend('Measured', 'Simulated');
elseif(labelMode == 'Chinese')
    xlabel('无人机飞行序列号')
    ylabel('SINR/(dB)');
    str = sprintf('UAV在200m高度，500m半径下的SINR比较');
    legend('实测', '仿真');
end
title(str)
grid on;
...
if(labelMode == 'English')
    xlabel('UAV flight sequence number')
    ylabel('SINR/(dB)')
    str = sprintf('Comparison of SINR for UAV at an Altitude of 200m and a Radius of 800m');
    legend('Measured', 'Simulated');
elseif(labelMode == 'Chinese')
    xlabel('无人机飞行序列号')
    ylabel('SINR/(dB)');
    str = sprintf('UAV在200m高度，800m半径下的SINR比较');
    legend('实测', '仿真');
end
title(str)
grid on;
...
if(labelMode == 'English')
    xlabel('UAV flight sequence number')
    ylabel('SINR/(dB)')
    str = sprintf('Comparison of SINR for UAV at an Altitude of 200m and a Radius of 1200m');
    legend('Measured', 'Simulated');
    str_filename = sprintf('Eng_h%d_r%d.png', h, 1200);
    % 设置坐标轴字体
...
elseif(labelMode == 'Chinese')
    xlabel('无人机飞行序列号')
    ylabel('SINR/(dB)');
    str = sprintf('UAV在200m高度，1200m半径下的SINR比较');
    legend('实测', '仿真'); 
    str_filename = sprintf('Chi_h%d_r%d.png', h, 1200);
    % 设置坐标轴字体
