classdef BSAntenna < handle
    ...
    methods   
        ...
        %% 构造函数
        function obj = BSAntenna(Nv,Nh,N_beam, phi_3dB, theta_3dB, tilt)
            %obj.max_antenna_gain =max_gain;
            obj.Nv=Nv;
            obj.Nh=Nh;
            obj.theta_3db=theta_3dB;
            obj.phi_3db = phi_3dB;
            obj.Am=30;
            obj.SLAv=30;
            obj.N_beam=N_beam;
...
%                 obj.phi_3db=90; %后续讨论，单排与多排波束赋型的3dB水平波宽与最大天线增益改成了一样。校准时多排的波束赋型中最大天线增益取值为7.5,3dB波宽取值为80度
            end
        ...
        function Ae=elementPattern(obj,theta,phi)
            ...
            for i = 1:size(theta, 1)
                for j = 1:size(theta, 2)
                    [phi2(i,j), theta2(i,j)] = miscUtils.global2local(phi(i,j), theta(i,j), 0, obj.gamma, 0);
                end
            end
            theta2 = theta2 - 90;
