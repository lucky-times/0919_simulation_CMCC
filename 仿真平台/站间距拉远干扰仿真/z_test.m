...
isd = 500:200:3000;
for i = 1:length(isd)
    SYS_config.ISD = isd(i);
    SYS_config.UE_height = SYS_config.ISD/2*tand(20)+25;
    [SIR(i),SINR(i)] = NR_sim_main(SYS_config); %运行主程序
end
figure(2)
plot(isd, SIR, 'r-' , LineWidth=2)
hold on;
plot(isd, SINR, 'g-.', LineWidth=2)
hold off
xlabel('站间距/米')
ylabel('SINR/dB')
title('SINR随站间距的关系')
legend('SIR', 'SINR', Location='NorthEast')
